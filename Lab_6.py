...
def func_y(x1=0, x2=0, x3=0):
    return round(9.1 + 2.2 * x1 + 5.9 * x2 + 7.9 * x3 + 5.6 * x1 * x1 + 1.0 * x2 * x2 + 3.0 * x3 * x3 +
                 2.2 * x1 * x2 + 0.8 * x1 * x3 + 8.8 * x2 * x3 + 3.6 * x1 * x2 * x3 + random.randint(0, 10) - 5, 2)
...
class Calc:
        ...
        if self.N == 4:
            ...
            for i in range(self.N):
                ...
                for j in range(self.m):
                    self.matrix_y[i].append(func_y(*self.matrix_x_natural[i][:3]))
    ...
    def initial_format(self):
        if self.N == 4:
            self.string_main1 = "№ " + "{:^10} " * self.amount_of_x + "{:^14}" *(
                    len(self.main_caption) - self.amount_of_x)
            self.string_main2 = "{} " + "| {:^6} | " * self.amount_of_x + "|{:^11}| " *(
                    len(self.main_caption) - self.amount_of_x)
        elif self.N == 8:
            self.string_main1 = "№ " + "{:^10} " * 3 + "{:^12} "*3 + "{:^13}" + "{:^14}" *(
                    len(self.main_caption) - self.amount_of_x)
            self.string_main2 = "{} " + "| {:^6} | " * 3 + "| {:^8} | " * 3 + "| {:^8} | " + "|{:^11}| " *(
                    len(self.main_caption) - self.amount_of_x)
        else:
            self.string_main1 = "№ " + "{:^10} " * 3 + "{:^12} " * 3 + "{:^13}"+ "{:^12} " * 3 + "{:^14}" *(
                    len(self.main_caption) - self.amount_of_x)
            self.string_main2 = "{} " + "| {:^6} | " * 3 + "| {:^8} | " * 3 + "| {:^8} | " + "| {:^8} | " * 3 \
                                + "|{:^11}| " *(len(self.main_caption) - self.amount_of_x)

        self.string_y_average1 = "\nСередні значення функції відгуку\n " + "{:^9}  " * self.N
        self.string_y_average2 = "|{:^8}| " * self.N
        if self.N == 4:
        ...
        self.string_dispersion2 = "|{:^7}| " * self.N
    ...
    def add_y(self):
        for i in range(self.N):
            self.matrix_y[i].append(func_y(*self.matrix_x_natural[i][:3]))
            if self.matrix_out:
                self.matrix_out[i].append(func_y(*self.matrix_x_natural[i][:3]))
        self.m += 1
        self.table_update()
        self.find_coefficients()
