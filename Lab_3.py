...
class Calc:
    def __init__(self, main_matrix, m):
        ...
        self.string_student_format, self.string_student_regression_caption = None, None
    ...
    def initial_format(self):
        ...
        self.string_student_regression_caption = "Тепер рівняння регресії має вигляд\nY = "
        self.string_regression_caption, self.string_student_regression = "", ""
    ...
    def out_regression_student(self):
        if sum(self.list_b) == 0:
            print(self.string_student_regression_caption + "0")
        elif sum(self.list_b[1:]) == 0:
            print(self.string_student_regression_caption + str(round(self.list_b[0], 3)))
        else:
            if self.list_b[0]:
                self.string_student_regression_caption += "{:.3f}"
            ...
            for i in range(1, N):
                if self.list_b[i]:
                    self.string_student_regression_caption += "{:{sign}.3f}*x" + str(i)
            ...
            print(self.string_student_regression_caption.format(*list_out, sign="+"))
            for row in range(N):
