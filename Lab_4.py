...
"""Додати нові змінні для 4 лаби, додати в перевірках критеріїв умову, щоб перевіряти: або лінійну регресію, або 
регресію з взаємодією, створити нові функції для виводу і ініціалізуючих обчислень для регресії взаємодії, створити 
функцію заміни стовпчика для 8 рядків, зробити вивід коефіцієнтів і після критерію Стьюдента зробити вивід рівняння з 
значимими коефіцієнтами, можливо також зробити нову функцію обчислення значення функції відгуку, не забути про різні 
повернення в алгоритмі, трошки змінити назви вже існуючих зміннних для того, щоб менше плутатись, спробувати автоматизу-
вати функцію знаходження коефіцієнтів рівняння, зробити два види повторень m
"""


#  НА ФУНКЦІОНАЛ 4 ГОДИНИ, НЕ БІЛЬШЕ!!!!!!!!!!!!!!!
#  Найголовніше переробити знаходження коефіцієнтів, інше просто(вивід теж можна швидко зробити, бо він вже якби є)
#  ПРАЦЮЙ ЕФЕКТИВНО І В ТЕБЕ ВСЕ ВИЙДЕ!!!!!!!
...
class Calc:
        ...
        if self.N == 4:
            ...
            for i in range(self.N):
                ...
                for j in range(self.amount_of_x):
                    ...
                    self.matrix_x_norm[i].append(
                        (self.matrix_x_natural[i][j] - self.list_x0[j]) /(self.list_delta[j]))
    ...
    def find_coefficients(self):
        ...
        if self.N == 4:
            ...
            self.list_b_natural = self.coefficients_interaction(self.matrix_x_natural, True)
            self.out_regression_caption(True)
            self.list_b_norm = self.coefficients_interaction(self.matrix_x_norm, False)
            self.out_regression_caption(False)
    ...
    def coefficients_interaction(self, matrix, flag):
        ...
        if flag:
            ...
            for i in range(self.N):
                ...
                for j in range(self.N):
                    matrix_help.append([reverse_matrix[col][j] * mult[j] for col in range(self.N + 1)])
    ...
    def check_fisher(self):
        self.d = self.N - self.list_coefficients.count(0)
        if self.N - self.d == 0:
    ...
    def out_regression_student(self):
        if sum(self.list_coefficients) == 0:
            ...
            list_out = [self.list_coefficients[i] for i in range(self.N) if self.list_coefficients[i] != 0]
...
while True:
    ...
    if not calc_lineal.check_adequacy:
        ...
        if calc_interaction.check_adequacy:
            break
    else:
        break